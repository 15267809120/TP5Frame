...
class Base extends Controller
{
	...
	public function __construct(){
		parent::__construct();
		$this->action = strtolower($_SERVER['PATH_INFO']);//dump(Session::get());
	}
	...
	public function isPower(){
		...
		//首页都可以进入
		if(strpos($this->action, '/adminer/index/index')) return ;
		foreach($menu as $key => $value){
			if(!empty($value['link']) && strpos($this->action, $value['link']) !== false){
